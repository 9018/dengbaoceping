...
for d, l2s in BASE_L2.items():
    for l2 in l2s:
        agg[(d, l2)] = {'domain': d, 'level2': l2, 'general_count': 0, 'cloud_count': 0, 'total_count': 0}

for r in clean:
    key = (r['domain'], r['level2'])
    if key not in agg:
        agg[key] = {'domain': r['domain'], 'level2': r['level2'], 'general_count': 0, 'cloud_count': 0, 'total_count': 0}
